...
class RestaurantDetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            is_favorite = (
                models.Favorite.objects.filter(customer=user)
                .filter(restaurant=models.Restaurant.objects.get(pk=pk))
                .exists()
            )

        total_seats = (
            models.DiningTable.objects.filter(restaurant=restaurant).aggregate(
                total=Sum("max_people")
            )["total"]
            or 0
        )
    ...
    def post(self, request, **kwargs):
        ...
        if is_favorite:
            models.Favorite.objects.filter(customer=user).filter(
...
class RestaurantListView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if int(price_session) > 0:
            restaurant_list = restaurant_list.filter(
                min_price__lte=price_session, max_price__gte=price_session
            )
        ...
        elif select_sort_session == "price2":
            restaurant_list = restaurant_list.order_by("-max_price")
        else:
...
class FavoriteListView(SubscriptionRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        queryset = models.Favorite.objects.filter(customer_id=user_id).order_by(
            "-created_at"
        )
        return queryset
...
class ReservationListView(SubscriptionRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        queryset = models.Reservation.objects.filter(
            customer_id=self.request.user.id, is_booked=True, is_dependent=False
    ...
    if pk:
        try:
            ...
            current_datetime = datetime.combine(
                reservation.date, reservation.time_start
            )
...
class ReviewListView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        restaurant = models.Restaurant.objects.filter(id=pk).first()
        is_posted = False
        if self.request.user.is_authenticated and self.request.user.account_type == 1:
            is_posted = (
                models.Review.objects.filter(customer=self.request.user)
...
class ReviewCreateView(SubscriptionRequiredMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        restaurant.review_num = models.Review.objects.filter(
            restaurant=restaurant, display_masked=0
        ).count()
...
def review_delete(request):
    if not getattr(request.user, "is_subscribed", False):
        return JsonResponse(
            {"is_success": False, "message": "User not subscribed"}, status=403
        )
...
class RestaurantListView2(OwnerRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for restaurant in restaurants:
            ...
            restaurant.reservation_count = restaurant.reservation_set.filter(
                date__gte=today, is_booked=True, is_dependent=False
            ).count()
            restaurant.menu_count = restaurant.menu_set.count()
            restaurant.replies_count = models.Review.objects.filter(
...
class RestaurantCreateView(generic.CreateView):
    ...
    def form_valid(self, form):
        # shop_ownerは店舗オーナーならその人、システム管理者ならフォームで指定した人
        user = self.request.user
        if user.is_authenticated and user.is_subscribed and user.account_type == 2:
    ...
    def get_success_url(self):
        ...
        if user.is_authenticated and user.is_subscribed and user.account_type == 2:
            return reverse_lazy(
                "restaurant_list_2", kwargs={"pk": self.request.user.pk}
            )
        if user.is_authenticated and user.account_type == 3:
...
class RestaurantUpdateView(generic.UpdateView):
    ...
    def form_valid(self, form):
        # Ensure that the shop_owner remains as the current user
        form.instance.shop_owner = self.request.user
        return super().form_valid(form)
...
class DiningTableUpdateView(OwnerRequiredMixin, generic.UpdateView):
    ...
    def get_success_url(self):
        ...
        restaurant_id = self.kwargs["restaurant_id"]
        return reverse_lazy(
            "dining_table_list", kwargs={"restaurant_id": restaurant_id}
        )
...
def dining_table_delete(request, restaurant_id, pk):
    ...
    if request.method == "POST":
        ...
        existing_reservations = table.reservations.filter(
            Q(is_booked=True) | Q(is_booked__isnull=False)
        )
        ...
        if existing_reservations.exists():
            ...
            table.delete()
            query_string = urlencode(
                {"message": "テーブルを削除しました。", "type": "success"}
            )

        # Redirect with the appropriate message
        return redirect(
            f"{redirect('dining_table_list', restaurant_id=restaurant_id).url}?{query_string}"
        )
...
class MenuUpdateView(OwnerRequiredMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        min_price = models.Menu.objects.filter(restaurant=restaurant).aggregate(
            Min("price")
        )["price__min"]
        max_price = models.Menu.objects.filter(restaurant=restaurant).aggregate(
            Max("price")
        )["price__max"]
...
class MenuCreateView(OwnerRequiredMixin, generic.CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        context["restaurant"] = models.Restaurant.objects.get(
...
class MenuDeleteView(OwnerRequiredMixin, generic.DeleteView):
    ...
    def get_success_url(self):
        ...
        )  # Get the restaurant ID from the object being deleted
        return reverse_lazy("menu_list", kwargs={"restaurant_id": restaurant_id})
...
class ReviewUpdateView2(OwnerRequiredMixin, generic.UpdateView):
    ...
    def get_success_url(self):
        ...
        elif user.is_authenticated and user.account_type == 3:
            return reverse_lazy("review_list_3")
...
def toggle_display_masked(request, pk):
    ...
    if request.user.is_authenticated and request.user.account_type == 3:
        return redirect(reverse("review_list_3"))
    elif request.user.is_authenticated and request.user.account_type == 2:
...
class ReservationManagementView(OwnerRequiredMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        dining_table = (
            get_object_or_404(models.DiningTable, id=dining_table_id)
            if dining_table_id
            else None
        )
...
class ReservationSlotCreateView(View):
    ...
    def get_context_data(self, **kwargs):
        ...
        restaurant = get_object_or_404(
            models.Restaurant, id=self.kwargs["restaurant_id"]
        )
        tables = models.DiningTable.objects.filter(restaurant=restaurant)

        # Generate 30-minute time slots from 9:00 to 22:30
        start_time = timezone.datetime.combine(
            timezone.now(), timezone.datetime.min.time()
        ) + timedelta(hours=9)
        time_slots = [
            (start_time + timedelta(minutes=30 * i)).time() for i in range(28)
        ]
    ...
    def post(self, request, *args, **kwargs):
        ...
        while current_date <= end_date:
            for table_id in selected_tables:
                ...
                for i, duration in enumerate(duration_mins):
                    ...
                    if reservation:
                        if (
                            not reservation.is_booked
                            and reservation.duration_min != duration
                        ):
                            reservation.duration_min = duration
                            updated_slots.append(reservation)  # Collect for bulk update
                            print(
                                f"Updated {table.name_for_customer} at {slot_time} on {current_date}"
                            )
                        else:
                            print(
                                f"Slot already booked for {table.name_for_customer} at {slot_time} on {current_date}"
                            )
                    else:
                        # Create a new reservation if it doesn't exist
                        created_slots.append(
                            models.Reservation(  # Collect for bulk create
                                restaurant=restaurant,
                                dining_table=table,
                                date=current_date,
                                time_start=slot_time,
        ...
        with transaction.atomic():
            ...
            if updated_slots:
                models.Reservation.objects.bulk_update(
                    updated_slots, ["duration_min"]
                )  # Bulk update
...
class RestaurantListView3(SuperuserRequiredMixin, generic.ListView):
    ...
    def get_ordering(self):
        # Get the ordering parameter from the request and set default to 'id'
    ...
    def get_queryset(self):
        ...
        if query:
            queryset = queryset.filter(
                ...
                | Q(shop_owner__user_name__icontains=query)
...
class ReviewListView3(SuperuserRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if query:
            queryset = queryset.filter(
                Q(customer__user_name__icontains=query)
                | Q(comment__icontains=query)
                | Q(restaurant__shop_name__icontains=query)
