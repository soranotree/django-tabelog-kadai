...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not shop_owners.exists():
            self.stdout.write(
                self.style.ERROR(
                    "No shop owners found. Please create some shop owners first."
                )
            )
            return
        ...
        for _ in range(100):
            ...
            restaurant = Restaurant.objects.create(
                ...
                description=fake.sentence(
                    nb_words=10
                ),  # Generate a Japanese sentence with approximately 10 words
                ...
                close_day_of_week=fake.random_element(
                    ["月", "火", "水", "木", "金", "土", "日"]
                ),
                created_at=now(),
                updated_at=now(),
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully created restaurant {restaurant.shop_name}"
                )
            )
