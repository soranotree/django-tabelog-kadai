...
from restaurant.models import Favorite, Restaurant
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        customers = CustomUser.objects.filter(
            account_type=1
        )  # Select customers with account_type = 1
        restaurants = Restaurant.objects.all()

        if not customers.exists():
            self.stdout.write(
                self.style.ERROR("No customers found with account_type=1.")
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Total {created_count} favorite records created successfully!"
            )
        )
