...
from restaurant.models import DiningTable, Restaurant
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not restaurants.exists():
            self.stdout.write(
                self.style.ERROR(
                    "No restaurants found. Please create some restaurants first."
                )
            )
            return
        ...
        for restaurant in restaurants:
            ...
            for i in range(1, 11):  # Add 5 dining tables per restaurant
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully created dining table {dining_table.name_for_internal} for restaurant {restaurant.shop_name}"
                    )
                )
