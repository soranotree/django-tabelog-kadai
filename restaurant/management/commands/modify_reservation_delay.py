...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR("Invalid input. Please enter a valid integer.")
            )
            return

        # Delay the reservation dates by the specified number of days
        updated_reservations = Reservation.objects.update(
            date=F("date") + timedelta(days=days)
        )

        # Print out a success message
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully delayed reservation dates by {days} days. {updated_reservations} reservations updated."
            )
        )
