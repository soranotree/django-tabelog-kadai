...
class Restaurant(models.Model):
    ...
    email = models.CharField(
        max_length=128, null=True, blank=True, verbose_name="メールアドレス"
    )
    ...
    business_time = models.CharField(
        verbose_name="営業時間", max_length=64, null=True, blank=True
    )
    close_day_of_week = models.CharField(
        verbose_name="定休日", max_length=32, null=True, blank=True
    )
    rate = models.FloatField(verbose_name="レート", default=0.0)
    rate_star = models.FloatField(
        verbose_name="レートスター", default=0.0, null=True, blank=True
    )
...
class DiningTable(models.Model):
    ...
    name_for_internal = models.CharField(
        verbose_name="卓名（店舗管理用）", max_length=64
    )
    name_for_customer = models.CharField(verbose_name="卓名（HP表示用）", max_length=64)
    min_people = models.IntegerField(
        verbose_name="最少人数", choices=NUMBER_OF_PEOPLE, default=""
    )
    max_people = models.IntegerField(
        verbose_name="最大人数", choices=NUMBER_OF_PEOPLE, default=""
    )
    created_at = models.DateTimeField(verbose_name="作成日時", auto_now_add=True)
    updated_at = models.DateTimeField(verbose_name="更新日時", auto_now=True)
...
class Reservation(models.Model):
    ...
    menu = models.ForeignKey(
        Menu, verbose_name="メニュー", on_delete=models.PROTECT, null=True, blank=True
    )
    date = models.DateField(verbose_name="予約日")
    time_start = models.TimeField(
        verbose_name="予約開始時間", choices=TIMES, default=""
    )
    duration_min = models.IntegerField(
        verbose_name="制限時間（分）", null=True, blank=True
    )  # 制限時間（分、整数値）
